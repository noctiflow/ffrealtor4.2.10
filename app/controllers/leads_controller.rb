...
class LeadsController < EntitiesController
  ...
  def index
    ...
    #   @leads = get_leads(page: params[:page]).order(created_at: :desc)
    # else
    #   @leads = get_leads(page: params[:page]).order(created_at: :desc).where(:assigned_to => current_user)
    # end

    # @buyers = get_leads(page: params[:page], per_page: params[:per_page]).where(:buysell => "Buyer")
    # @sellers = get_leads(page: params[:page], per_page: params[:per_page]).where(:buysell => "Seller")
